...
- (instancetype)init
{
    ...
    {
        self.hud = [[[MMHud alloc] init] autorelease];
...
- (void)_buildHUDWindow
{
    if (self.window == nil)
    {
        self.window = [[[MMProgressHUDWindow alloc] init] autorelease];
        
        if (self.presentationViewController == nil)
        {
            self.presentationViewController = [[[MMProgressHUDViewController alloc] init] autorelease];
