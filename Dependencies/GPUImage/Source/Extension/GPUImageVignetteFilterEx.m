...
NSString *const kOCMVignetteFragmentShaderString = SHADER_STRING
(
 ...
 uniform lowp float vignetteLevel;
 ...
 void main()
 {
     ...
     lowp float percent = smoothstep(vignetteStart, vignetteEnd, d) * vignetteLevel;
...
@implementation GPUImageVignetteFilterEx
...
@synthesize vignetteLevel = _vignetteLevel;
...
- (id)init;
{
    if (!(self = [super initWithFragmentShaderFromString:kOCMVignetteFragmentShaderString]))
    {
		return nil;
    }
    ...
    vignetteLevelUniform = [filterProgram uniformIndex:@"vignetteLevel"];
...
- (void)setVignetteLevel:(CGFloat)newValue
{
    _vignetteLevel = newValue;
    
    [self setFloat:_vignetteLevel forUniform:vignetteLevelUniform program:filterProgram];
}
